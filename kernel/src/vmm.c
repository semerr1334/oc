...
static u64 walk_create(u64 table_phys, u64 idx)
{
    ...
    for (int i = 0; i < 512; i++)
        nt[i] = 0;
    t[idx] = frame | PTE_PRESENT | PTE_WRITE | PTE_USER | PTE_GLOBAL;
    return frame;
}
...
void vmm_init(void)
{
    ...
    /* идентично: 0..64GiB страницами по 2MiB (PDPT[0..63]) — как в UEFI-загрузчике,
     * чтобы фреймбуфер (может быть выше 4 GiB на некоторых ноутах) остался доступен.
     * 64 PD = 64 кадра, запас есть. */
    ...
    for (int i = 0; i < 64; i++) {
        ...
        for (int j = 0; j < 512; j++)
            pd[j] = 0;
        pdpt[i] = pd_phys | PTE_PRESENT | PTE_WRITE | PTE_USER | PTE_GLOBAL;
        /* куча ядра 16 GiB..16 GiB+64 MiB — не мапим huge, чтобы kmalloc мог
         * создавать 4 KiB страницы через vmm_map_page */
        if ((u64)i == (HEAP_BASE >> 30))
            continue;
        for (int j = 0; j < 512; j++) {
            u64 pa = (u64)i * 0x40000000ULL + (u64)j * PAGE_2M;
            pd[j] = pa | PTE_PRESENT | PTE_WRITE | PTE_USER | PTE_HUGE | PTE_GLOBAL;
        }
    }
