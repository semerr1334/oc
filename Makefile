...
KERNEL_S_SRCS := kernel/src/entry.S kernel/src/isr.S kernel/src/ramdisk.S
...
# ---- программы .ocp (flat, вход по адресу 0x2000000) и ramdisk ----
OCP_CFLAGS = -Os -ffreestanding -fno-stack-protector -fno-builtin -fno-pie \
             -nostdlib -Wl,-T,scripts/ocp.ld -Wl,--oformat=binary \
             -Wl,-e,_entry

$(BUILD)/prog/hello.ocp: programs/hello.c programs/ocp.h | $(BUILD)/prog
	gcc $(OCP_CFLAGS) $< -o $@

$(BUILD)/prog/wiki.ocp: programs/wiki.c programs/ocp.h | $(BUILD)/prog
	gcc $(OCP_CFLAGS) $< -o $@

$(BUILD)/prog/%.ocp: programs/%.c programs/ocp.h | $(BUILD)/prog
	gcc $(OCP_CFLAGS) $< -o $@

$(BUILD)/ramdisk.bin: scripts/packfs.py $(BUILD)/prog/hello.ocp \
                      $(BUILD)/prog/wiki.ocp $(BUILD)/prog/calc.ocp \
                      $(BUILD)/prog/game.ocp $(BUILD)/prog/snake.ocp \
                      $(BUILD)/prog/piano.ocp programs/notes.txt | $(BUILD)/prog
	python3 scripts/packfs.py $@ $(BUILD)/prog/calc.ocp \
	    $(BUILD)/prog/game.ocp $(BUILD)/prog/snake.ocp \
	    $(BUILD)/prog/piano.ocp $(BUILD)/prog/hello.ocp \
	    $(BUILD)/prog/wiki.ocp programs/notes.txt

$(BUILD)/k/ramdisk.o: kernel/src/ramdisk.S $(BUILD)/ramdisk.bin
